...
class Pins:
    ...
    def printPins(self):
        ...
        for pin in self.__outputs:
            ...
            print ('### Pin ' + str(pin) + ' is setup')
            # Right Wheel Forward
        rmfPWM = gpio.PWM(wheels.rmf,100)  # Right Motor Forward, sets power output to 100
        rmbPWM = gpio.PWM(wheels.rmb,100)  # Right Motor Back, sets power output to 100
        ...
        rmbPWM.ChangeDutyCycle(0)   # Right Motor Back has 0 recieving power as we dont want it to go back
            # Left Wheel Forward
        lmfPWM = gpio.PWM(wheels.lmf,100)  # Left Motor Forward, sets power output to 100
        lmbPWM = gpio.PWM(wheels.lmb,100)  # Left Motor Back, sets power output to 100
